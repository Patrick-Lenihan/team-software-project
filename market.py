...
class MultiLevelQueue(object):
	...
	def add(self,bid):
		...
		'''if level > len(self._top_level_list)-1:
		...
		if len(self._top_level_list) <= level:
			for i in range(level - (len(self._top_level_list)-1)):
				self._top_level_list.append(PQ())
		#print("ahhh",self._top_level_list)
		#print("level: ",level)
	...
	def selectWinners(self,prediction,winners):
		...
		for level in self._top_level_list:
			...
			if total <0:
				...
				total = 0 
			#print("winner.producer: ",winner.producer,"winner: ",winners.keys())
			...
			if total ==0:
				...
				for producer in winners.keys():
					if len(winners[producer])< len(winners[winner.producer]):
						winners[producer].append(None)
		print("the winners: ",winners)
		return winners
		'''
		total = prediction
		#for level in range(len(self._top_level_list)):
		level = 0
		while True:
			#print("-------------------------level: ",level)
